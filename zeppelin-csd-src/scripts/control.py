...
def generate_livy_conf_struct(spark_version, interpreter_id, interpreter_name, livy_server_properties, interpreter_json,
    ...
    conf["properties"]["zeppelin.interpreter.localRepo"] = "%s/local-repo/%s" % (
...
def start():
    ...
    livy2_conf_struct = generate_livy_conf_struct("spark2", "2CYCWDZPP", "livy", server_props_string,
                                                  livy_interpreter_json_string, livy_properties_string)
    ...
    run_command("hdfs dfs -mkdir -p %s" % os.environ["ZEPPELIN_CONF_DIR"])

    run_command("hdfs dfs -put -f %s %s" % (completed_filename, os.environ["ZEPPELIN_CONF_DIR"]))
    ...
    run_command("%s/bin/zeppelin.sh" % os.environ["ZEPPELIN_HOME"])
    return
